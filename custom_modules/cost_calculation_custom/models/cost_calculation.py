...
class cost_calculation_custom_0(models.Model):
    ...
    x_studio_fecha_reunion_montador = fields.Datetime(string="Fecha Reunión Montador")
                ...
                for partner in lead.partner_ids:
                    if(partner.user_ids):
                        ...
                        if(tipo == 'Medidor'):
                            ...
                            self.x_studio_fecha_reunion_medidor = lead.start_date if lead.start_date else lead.start_datetime
                ...
                for partner in lead.partner_ids:
                    if(partner.user_ids):
                        ...
                        if(tipo == 'Montador'):
                            ...
                            self.x_studio_fecha_reunion_montador = lead.start_date if lead.start_date else lead.start_datetime
                        elif(tipo == 'Montador plaza' or tipo == 'Partner plaza'):
                            self.x_studio_montador = partner.parent_id.id if partner.parent_id else partner.id
                            self.x_studio_fecha_reunion_montador = lead.start_date if lead.start_date else lead.start_datetime
    ...
    def save_cost(self):
        ...
        is_purchase_montador = True if ((self.x_studio_obra == 'si' and self.x_studio_coste_montaje > 0) or (self.x_studio_montaje == "si") or self.x_studio_desmontar > 0 or self.x_studio_post_cuarzo > 0) else False
        #is_purchase_montador = True if (self.x_studio_obra == 'si' and self.x_studio_coste_montaje > 0) else False
        ...
        purchase_montador = self.crud_purchase_order_line(purchase_montador, categoria_costes, producto_desmontar, self.x_studio_desmontar, True)
        purchase_montador = self.crud_purchase_order_line(purchase_montador, categoria_costes, producto_post_cuarzo, self.x_studio_post_cuarzo, True)
